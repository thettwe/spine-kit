...
**Version:** 0.11 · **Status:** design frozen — build reference · **Owner:** _assign before adoption_
...
>
> **v0.10** answers a fourth adversarial design review (`docs/reviews/2026-08-26-codex-adversarial-review-v0.9.md`), each version answering the review of the one before. v0.7 made the three central guarantees — rebuildable provenance, frozen tests, safe auto-merge — mechanically enforceable; v0.8 made the text stop claiming more than the mechanism delivered; v0.9 removed the modes in which a guarantee could quietly not hold; v0.10 finishes that by **deleting two mechanisms rather than repairing them**. Auto-merge no longer exists at all under the default threat model (`C-A3: threat.candidate = hostile`, §7.4 rule 5), because nothing establishes that a hostile candidate's own runner told the truth — and §7.4 now states the guarantee at its true size: provenance and blast radius, never correctness. Deferred reconstruction is gone, because a deferred proof's failure was never a git object (§6.3). A rollback's managed set is enumerated from the manifests rather than the diff (§7.5, G16). §12 maps every finding to where it is closed — and lists what is not. v0.11 adds one thing that is not a review answer: the OKF projection (§6.8), an export rendering of the derived graph. §11 is the vocabulary every section shares: trailer names, key roles, states, gates, CLI flags. Where prose and §11 disagree, §11 wins.
...
- **Rules carry IDs, and rules grow teeth. ⚙** Every rule is numbered (`C-1`, `C-2`…) and optionally carries an `enforced_by:` field pointing at a real check — reported by `spine check --constitution`, which runs repository code and therefore never runs in the trusted stage (§11), so a team's own rules are a health report and never a landing gate; only the twelve scaffolded rules below, enforced by spine's own gates, can block a landing — a lint rule, a dependency constraint, a grep probe, or an LLM-judge for the genuinely fuzzy ones. `spine check --constitution` reports each rule as **enforced** or **aspirational** (a failing `enforced_by` check on `T` is a wire; for `C-T3` it is `class=protected`, never warn mode), and the enforced ratio is a health metric: a constitution that is mostly aspirational is a wish list, not a constitution.
...
C-A3: threat.candidate = hostile          # hostile ⇒ auto-merge is unavailable (§7.4)   enforced_by: spine:G11
...
- `C-M4: merge.auto = on` **and** every precondition of §7.4 rule 5 holds for this run — beginning with `C-A3: threat.candidate = trusted`. Either missing → a wire, and the class picks the state: `G11` (`C-M4`) is a `class=tripwire` wire to `landing-review` where the constitution says off; a failed precondition is a `class=protected` `G1` wire to `protected-review`, reviewer ≠ signer in team mode.
...
| tests-approved | wire tripped on `T` (Drift / Freshness / Strength, or `C-M4` off in the constitution) | landing-review | tripwires (§5.2) |
| tests-approved | `C-M4` computed off by a failed §7.4 rule 5 precondition — the threat model, the profile, the key-visibility assertion, or the CAS | protected-review — `class=protected` `G1` naming the precondition, reviewer ≠ signer in team mode | G1 · G11 |
...
| base-moved † | runs exhausted | starved — reported, and terminal until a human review resets the count (§5.4); never re-queued on its own, or `C-M3` bounds nothing | G11 |
...
Corollary — **the provenance law**: every node and edge must cite its source. An edge that cannot say where it came from does not exist. The provenance grammar is fixed: `<path>:<line>` · `git:<sha>` · `git:<sha>:msg:L<n>` (a line inside a commit message) · `git:<sha>:trailer:<Name>` · `git:<sha>:patch-id` · `git:<sha>:<path>:<line>` (a line of a file at a commit) · `spine:<version>:floor` (the protected floor shipped in the release). This is what makes the graph auditable, regenerable, and honest — and what G10 proves on a clean clone before every landing is pushed. A *rendering* of the graph — a PR body, a report note, an OKF bundle (§6.8) — is not a second graph and is not exempt from this law: it must transport each node's and each edge's `src` verbatim in the grammar above, and nothing in spine may read one back. **A rendering that is read is a graph**, and the law then applies to it in full.
...
| Freshness | G11 — Base currency | A gate record is consumable only while trunk == `base` and the branch == `head`; the ref update is the check. A lost CAS → `base-moved`; re-verification depth per `C-M2`; runs per `C-M3`, then `starved`; `C-M4` evaluates `on` only when every precondition of §7.4 rule 5 holds in this run — including `C-A3: threat.candidate = trusted`, without which auto-merge does not exist for the repo — and `off` ⇒ a landing-review wire on every landing. Sign-off survives; approval survives iff frozen blobs are byte-identical (or trunk's); a review survives per the §5.4 table; gate results never survive | no |
...
### 6.8 The OKF projection ⚙ — knowledge that leaves without the toolkit
...
| `adr` · `constitution` | `/adr/ADR-007.md` · `/constitution/v3.md` | `spine.adr` · `spine.constitution` | the ADR's authored `Date:` is the one honest `timestamp` in the bundle, and it is an authored claim, not a hashed one |
...
5. **Auto-merge is a capability, not a preference.** `C-M4: merge.auto = on` is a request; whether a run may act on it is computed, per run, from these preconditions — each read from trunk or produced by this run, never asserted by the branch asking to merge:
   ...
   A landing that runs no suite — a tombstone or a reseal (§5.5) — is exempt from preconditions 1 and 2 and seals `profile=n/a`; it has no test outcome to attest and never claimed one. Any precondition missing and the run evaluates `C-M4` as `off` whatever the constitution says, and raises a **`G1` wire of `class=protected`** naming which precondition failed — reviewer ≠ signer in team mode, `reason=` mandatory and required to name that precondition. That is the difference between pointing a review at the test outcomes and merely hoping it looks: the class is machinery G13 and G14 already enforce, where a sentence about reviewer attention would have been the discipline §1 forbids. The profile is sealed into the landing (`profile=` on `Spine-Seal`), so the ledger says, for every landing forever, how strong the evidence behind its green suite was. This is one rule with three former patches folded into it: an unattested channel, a deferred reconstruction and a provider-made commit are the same failure — a landing whose central claim was never established before it became irreversible.
...
3. `spine context <id>` — session resume via graph query (§6.4), including Agent B's interface slice. `spine index --okf` ships beside it (§6.8): both are readings of the same graph, and an export is worth building only once the query it exports exists.
...
|---|---|---|
| Mandatory human gates | 1 judgement + 1 attestation | 1 judgement (intent sign-off) + 1 attestation (`--approve` signs B's verdict; the pipeline key takes it at roadmap 5) — reviews are conditional, on a wire or a floor hit |
...
| Graphs | 2 | 2 (traceability, code) + 1 transition table (~40 rows, all machine-side) + 1 rendering (the OKF projection — a cache of the traceability graph, never a source, §6.8) |
...
| `Spine-Review` + `-Sig` (repeatable) | review commit; copied into the landing — two required for a landing with no signer in team mode | `INT-042\|quick\|reseal class=tripwire\|protected\|break-glass head=<sha> tree=<oid> base=<sha> [intent=<oid>] report=sha256:<hex> wires=<G<n>[:path],…> reason="…" reviewer=<p>` — wires sorted; gates without a path use the bare id; `head=` is the content head `Hc` (§5.4); the first field is the seal's and `intent=` is present only when the landing has one; for `class=break-glass`, `wires=` lists the gates bypassed |
...
| `Spine-Gates` | landing | `G1=pass … G16=pass` — every gate that ran, never G10 (it runs after the seal); entries read `=override` where a signed review accepted the wire or break-glass bypassed the gate; a tombstone lists the four that ran |
...
**Files and refs.** `intents/<ID>.md` — only on `refs/heads/intent/<ID>`, deleted by the landing, never on trunk. Toolkit upgrades ride `refs/heads/spine/upgrade-<version>`; quick-lane candidates are `refs/heads/quick/*`. `.spine/manifest.json` (lockfile; its frozen fields — `manifest_version`, `cli`, `params.trunk`, `params.isolation`, `paths`, `files[]{path, owner, blob}` — never change name, type, `owner` set, or the meaning of a `paths` key, so any binary can judge any manifest, §6.7) · `.spine/allowed_signers` (keyring) · `.spine/ci.sh` (every provider — the collector's entry point, executed from trunk, §7.4 rule 0) · `.spine/cache/` (gitignored: `graph.sqlite`, `staging/`, `okf/` — the OKF projection of §6.8, never read back — and `results/<T>.jsonl` — the collector's output and the untrusted job's only artifact: first line `tree=<oid> base=<sha> tool=<version>+sha256:<dist_hash> keys_visible=false profile=container\|uid\|none ids=<n>` — fields are `key=value`, space-separated, order fixed, a repeated key rejects the file, and `profile=` names the boundary the collector *achieved*, derived from creating it, never from configuration that claims it, then the id set collected on `B`, then one record per runner-native id). Managed regions `AGENTS.md#spine`, `.gitignore#spine`, `.gitattributes#spine`. CI: `.spine/ci.sh` always, plus `.github/workflows/spine.yml` | a `.gitlab-ci.yml` snippet | a definition outside the repository. Not in the repo: the pre-receive hook, `refs/notes/spine` (optional report copy, never a source), git config `spine.trustRoot`, and the CI variables `SPINE_TRUST_ROOT` and `SPINE_PIPELINE_KEY` — the latter named normatively so the collector knows what it is asserting the absence of (§7.4 rule 0). No `.spine/prompts/`, no `prepare-commit-msg` hook.
...
**OKF `type` values.** The projection emits a closed set, namespaced so a consumer can tell them from anyone else's: `spine.intent` · `spine.acceptance-criterion` · `spine.adr` · `spine.constitution` · `spine.changeset` · `spine.signoff` · `spine.approval` · `spine.review` · `spine.reopen` · `spine.withdrawal` · `spine.upgrade` · `spine.signer` · `spine.test` · `spine.code-unit`. OKF's only required field is a free-form `type` with no registry, so this list is the registry for anything spine emits.
...
- `spine index [--fresh] [--dump] [--okf [<dir>]]` — `--okf` writes the projection of §6.8 and refuses a destination `git check-ignore` does not cover.
...
- **The OKF projection is unproved by design** (§6.8). G10 diffs `--dump`, not a markdown tree, so nothing in spine ever proves the emitter deterministic — which is exactly why nothing may read the bundle back. That rule is the machine-enforceable part, and it is enforced by the `git check-ignore` refusal. Two further costs are real: an edge's kind and `src` travel in prose beside each link rather than in a slot, so a consumer that misreads `declares` polarity inverts a tripwire; and OKF v0.1 is ten weeks old and self-described as unfinished, so a second consumer on a later version is the moment the emitter must be versioned through the manifest.
