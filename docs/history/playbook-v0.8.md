...
**Version:** 0.8 · **Status:** design frozen — build reference · **Owner:** _assign before adoption_
...
>
> **v0.8** answers an adversarial design review of v0.7 (`docs/reviews/2026-08-26-codex-adversarial-review-v0.7.md`), as v0.7 answered one of v0.6. v0.7 made the three central guarantees — rebuildable provenance, frozen tests, safe auto-merge — mechanically enforceable; v0.8 fixes where the text still claimed more than the mechanism delivered. Test results are produced by trunk's collector rather than the candidate's runner, and what remains unproven is stated exactly (§7.4). Reconstruction is proved *before* a landing is pushed, not after (§5.4 step 5, G10). Test-only code hiding inside `expected` is frozen instead of excused (§4.3). §12 maps every finding to where it is closed — and lists what is not. §11 is the vocabulary every section shares: trailer names, key roles, states, gates, CLI flags. Where prose and §11 disagree, §11 wins.
...
C-M5: merge.reconstruct = inline          # inline | scheduled:<window> — G10 before the push (§5.4) enforced_by: spine:G10
...
**What a green gate proves, exactly — stated, not hidden.** The collector of rule 3 is trunk's code, launched by trunk's job definition: it fixes the id set from `B` before the candidate's tree exists, and owns the result file and its header behind a uid no candidate process holds. Where that second uid exists, a candidate can no longer forge a result *file*, hide a landed test by making it uncollectable, or present one tree's results as another's; where it does not, none of that holds and G1 reports `unproven`. What remains, even at full strength, is the honesty of the collector's own descendants. Frozen tests must import the code under test, so on the `T` run the candidate's code and the runner share a process, and code that monkeypatches the assertion library or the reporter can lie on the stream the collector faithfully records. **G1's `passed` is therefore exactly as strong as the isolation between a candidate's code and its own runner, and nothing in this design establishes that property.** The bar-raisers are real and none of them closes it: the read-only harness (G8), `C-T3`, the sandbox, the no-new-dependency wire, ingestion by tree id. **G6 is not the answer** — it runs in the same untrusted stage through the same collector, so a runner that lies about the suite lies about the mutants too; G6 measures weak tests, never a dishonest runner. The two things that would close it are refused in v1 with their reasons: out-of-process attestation of every test outcome does not exist for in-process runners, and a trusted stage that re-runs the suite itself would put candidate code inside the only stage holding a key (§7.1). This is the design's largest residual, and it is why `C-M4` ships `off` (§2.1) — a human sees every landing while a team decides whether it can live with it.
...
| `Spine-Seal` + `-Sig` | landing; the last `Spine-*` line | `INT-042\|quick\|reseal base=<sha> head=<sha> tree=<oid> report=sha256:<hex> tool=<version>+sha256:<dist_hash> git=<major.minor> mode=solo\|team\|recovery recon=inline\|scheduled envelope=sha256:<hex over every Spine-* line above, in order, LF-joined> signer=<p>` — the first field is the landing's identity: the intent id for a gated landing or tombstone, `quick` for a quick-lane landing **and for every toolkit lifecycle landing** (upgrade, rollback, uninstall, re-init — they ride the quick lane, §6.7), `reseal` for a reseal; `head=` is the content head `Hc` (§5.4); `tree=` names `L`'s tree, so G9 checks it from `L` alone |
...
v0.8 answers an adversarial design review of v0.7 (`docs/reviews/2026-08-26-codex-adversarial-review-v0.7.md`; verdict: needs-attention / no-ship). Where the v0.6 review attacked guarantees that were missing, this one attacked the mechanisms that were supposed to provide them. Each finding, and where it is closed:

| Finding | Severity | What was wrong | Closed by |
|---|---|---|---|
| Candidate-controlled test reports could authorize arbitrary code | critical | The untrusted stage ran the candidate's tests *and wrote the result file*; the trusted stage checked only that the file named the right tree, so a branch could report every frozen and trunk test id as passed while running nothing. G6 was named as the countermeasure although it reports through the same channel | The collector (§7.4 rule 3): `spine check --ci --collect`, trunk's pinned binary, installed and hash-verified by trunk's `.spine/ci.sh`, launched by a job whose *definition* is trunk's (rule 0 — a push-triggered job runs the candidate's file and could simply never call it). It fixes the id set on `B` before `T` exists, spawns the runner as a child, reaps the process group, and writes the result file behind a second uid no candidate process holds; where the sandbox cannot supply that uid the collector owns nothing, and the design says so — results are advisory, G1 reports `unproven`, `C-M4` is `off`. The `C-M4` test is per run, not a latch, because nothing in this design may remember a past run. G6 demoted from countermeasure to strength signal (§4.3, G6). What is *not* closed — an in-process runner lying on its own stream — is stated wherever the guarantee is claimed (§1.1, §7.4, §8), not only in §7.4 |
| Mutable expected-path oracles bypassed the blob freeze | high | The closure excluded everything under `expected`, so an oracle, golden or helper living there could be weakened while every `Spine-Frozen` blob stayed byte-identical, with no gate to notice | The closure walk now separates test-only code from code under test by inbound imports read **at the approval's `base=`** — the tree on trunk, which the branch cannot edit: a module that existed there and that no non-test file imported there is frozen as a leaf (§4.3). Anchoring at `base=` is what makes the rule both safe and usable: buying an exclusion means landing the importing line on trunk first, through a full gated intent, while a module that does not exist at `base=` is the stub the red tests import and stays excluded, so greenfield work is untouched. The read-only-harness clause was extended to landed frozen sets so the protection survives the intent that bought it. What survives — an oracle the implementation also imports, or one A creates on the branch — is a stated residual, not a claimed closure |
| Offline reconstruction was checked only after the irreversible CAS | high | G10 ran on the landed commit and "blocked nothing", so the first envelope the indexer could not reproduce would already be on trunk | G10 moved ahead of the compare-and-swap: step 5 of §5.4 pushes `L` into a scratch clone as that clone's trunk — never moving the runner's own refs, which would destroy the guard step 6 depends on where there is no remote — then indexes, clean-clones, dumps and diffs, and self-checks G9 over `L`. A mismatch refuses the push and ends the run `reconstruction-failed`: a deterministic failure, so it never re-queues and never spends a `C-M3` retry. `C-M5` says whether G10 runs inline or on a window, and the seal records which with `recon=`, so the ledger distinguishes a landing that was proved from one that was not. G10 still never appears in `Spine-Gates` — `L` exists by then and its seal covers its own message |
| Manifest schema upgrades had no executable transition path | high | "Every binary parses every future `manifest_version`" and "a bump is an upgrade the older binary refuses" contradicted each other, leaving the first bump unlandable by either path | The frozen fields' names, types and `owner` set are themselves invariant across versions (§6.7, §11), so the promise is keepable; a bump lands like any upgrade under the base's pin, and the skew table governs the *next* invocation rather than the landing (§6.7). A release that must change a frozen field is `--uninstall` and re-init, not a bump. The review's two-release bridge was refused: it would need a binary to validate semantics it has never seen |
| The normative trailer table made lifecycle events malformed | medium | §11 required `Spine-Intent` on every event but quick and reseal landings, while upgrade events, lifecycle landings and their reviews carry no intent id — so §11, which wins, rejected the commits §6.7 requires | The `Spine-Intent` row now names where the trailer appears rather than where it does not (§11); the seal's first field is stated as the landing's identity, with lifecycle landings sealing as `quick`; `approves` targets `cs:<L>` for id-less lines (§6.2); and `spine/upgrade-*` enters the transition table through the quick-candidate row it rides (§6) |

One further hole surfaced while closing the fourth: the recovery landing admitted any `Spine-Upgrade` whose `to=` was installable, so an *upgrade* — not just a rollback — could be sealed by two reviewers with no pipeline key and no trusted stage. A rollback is now identified structurally, by a `cli` object restored byte-for-byte from an ancestor's manifest, which `--rollback` satisfies by construction and a forward upgrade never can — no gate has to order two version strings — and the admitted forms are that, an uninstall (`to=none`), and a re-init (`from=none`) (§7.5, G15).

**What v0.7 closed**, from `docs/reviews/2026-08-25-codex-adversarial-review-v0.6.md`, which found rebuildable provenance, frozen tests and safe auto-merge unenforceable as specified: historical intents read from PR descriptions → the intent envelope sealed into the landing commit, `implements = M(L) = git rev-list B..L`, the G9 ledger, G10 reconstruction, reseals for orphans (§5.5, §6). Test immutability with no checkpoint → the signed approval record freezing the closure by blob id, a harness read-only from the branch after approval, G8, G12 (§4.3). Identity mistaken for authority → roles as SSH namespaces in a keyring read from trunk, the protected floor, a trusted job running trunk's definition with a key no candidate can reach, G13–G16 (§7). The merge-time race → the serialized merge protocol, records bound to git objects, `push --atomic --force-with-lease` as compare-and-swap, G11 (§5.4). Scaffolding with no lifecycle → the manifest lockfile, ownership classes, idempotent `init` as upgrade, atomic apply, path-based rollback, G15 and G16 (§6.7). Before freezing, v0.7 was itself re-reviewed by one skeptic per finding, a fresh holistic reviewer and consistency and voice critics, closing roughly a hundred and fifty further holes — among them a trusted job definable from a candidate branch, an upgrade with no binary able to evaluate it, review commits that moved the head they were bound to, a reseal that could not pass the ledger it exists to repair, and a two-signer team that could never revoke a key.

Like v0.7, v0.8 was adversarially verified before freezing — one skeptic per change, plus consistency, voice and fresh-implementer critics — and that pass rewrote three of the five closures rather than confirming them. The collector's trust boundary was a directory rather than a uid, and the job that ran it was still triggered from the candidate's own workflow file, so the forgery had moved rather than closed. G10's ref move would have made every remote-less landing unlandable for ever, by destroying the old-value guard the local transaction depends on — invisible wherever a remote exists, which is how it would have shipped. And the freeze closure, read over the branch's own tree, froze every greenfield stub while falling to one throwaway import. It also found that §5.4 step 3 — the procedure an implementer builds `--land` from — still said the trusted stage runs the test suite, the exact mechanism the first finding condemned. Those are fixed above. So are the smaller seams: `Spine-Intent` demanded on implementation commits, lifecycle landings left malformed by four neighbouring rows, uninstall locked out of recovery, G7's hard lease made warnable, and the result file absent from the normative file inventory it is gated on.

**Residual risks, stated rather than hidden** — in the order they should worry a team about to turn `C-M4` on:

- **A candidate's runner can lie about its own results** (§7.4). Where the sandbox gives the collector a second uid, forged files, relabelled trees and hidden ids are closed; nothing closes a runner sharing a process with the code it tests. Where it does not, nothing is closed at all and G1 says `unproven`. `C-M4` ships `off` for exactly this reason.
- **An oracle the implementation also imports can be weakened** without changing a frozen blob (§4.3): Agent B's job, G6's signal, nobody's gate.
- **A laptop or Windows agent with a live signing agent** can satisfy the TTY check (§7.1). A hardware key or `ssh-add -c` closes it; `spine init --status` names every key that is not hardware-backed.
- **Sybil solo, and signer ≠ diff-author** (§7.2): one human holding two keys is a statistical finding — `spine stats` reviewer diversity — never a mechanical one.
...
- **Provider-made landing commits** (§5.4, configuration (b)) cost the pipeline signature and, under squash, `H`; anything worse lands `unattested` and loudly.
...
- **G10 scheduled rather than inline** (§6.3), the option large repos are given: reconstruction is then proved over a window instead of at every landing, and §1.1's claim is weaker by exactly that window.

Also in v0.8: the collector is a flag, not a fifth command (§10); and the review's recommendation to require cryptographic attestation of every test outcome is refused as unbuildable for in-process runners — with the reason recorded in §7.4 rather than the finding quietly downgraded.
