...
**`params.isolation` is *not* monotone**, and the difference is worth stating because it looks like an oversight. Precondition 1 of PB §7.4 rule 5 reads it from trunk on every run and compares it with the ingested header's `profile=`; lowering it weakens the *next* run and latches nothing open. `params.langs` is different because the guarantee it carries — a landed test is still collected — is retroactive. What the collector may write for either value is not this document's: `result-file.md` §7.1 fixes the mechanism, its host prerequisites, the probe and the three tests that license `profile=container`, and makes `"uid"` a refusal rather than a header value — which is why §6.2 check 12b keeps `"uid"` out of a landed tree in the first place.
...
|---|---|
| the two workflows and `.spine/ci.sh` (§8.1), and the three 1.3.0 renders (§8.6) | **not printed.** CI §3.3 refuses to invent a distribution hostname or a third party's action pin, so no conforming render exists to print. The counts and blobs are real for the stand-in bytes they were taken over; nothing normative reads them |
...
The workflow and `ci.sh` bytes below are **stand-ins**, and say so: CI §3.3 refuses to invent a distribution hostname or a third party's action pin, so a conforming render of `.spine/ci.sh` and the two workflows cannot be printed until `@@DIST_BASE@@` is fixed. The bytes are real, the blobs are computed from them, and the manifest is exact for *this* repository. `CONSTITUTION.md` is **not** a stand-in: it is CN §12.1's published document, and the blob below reproduces CN §12.2's.
...
- **The distribution host.** CI §5.5 fixes the layout and the byte format; the hostname is a release-checklist item and inventing one would publish a URL nobody serves.
