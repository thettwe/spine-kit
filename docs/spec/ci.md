...
The templates carry render-time tokens that `spine init` substitutes from constants embedded in the release:
...
**A rendered file still containing `@@` or a `PIN_` token is not a conforming render, and `init` refuses to write it.** The tokens appear in this document because inventing a distribution hostname or a third party's commit id would be publishing a digest nobody computed (§14 R5, §18 OPEN-1 and OPEN-7).
...
| `SPINE_DIST_BASE` | variable, not secret | both, optional | Overrides the release's baked distribution root (§5.5). Present so a repository can mirror the release; it changes no hash that is checked. |
...
- **In v1 nothing enforces it, and that is stated rather than implied.** The allocation above is right and the enforcer is absent: the only boundary v1 ships (RF §7.1, M1) is a filesystem-and-process boundary — mount, PID, IPC and user namespaces — and creates no network namespace, so a runner under it reaches whatever the job reaches. RF §14 OPEN-9 files it, names the fix as a restore phase in §6.1 followed by a loopback-only namespace, and records that nothing gates on it meanwhile: no gate, no auto-merge precondition and no result-file field reads egress.
- **The distribution host is not on the registry allowlist and must not be.** PB §7.1 grants the untrusted stage "an allow-listed registry proxy during dependency restore, then none" — and `ci.sh` reaches `SPINE_DIST_BASE` *before* dependency restore, to fetch the release it is about to verify. The two are different accesses with different lifetimes: the installer's fetch is authenticated by a digest read from trunk and happens before any repository code exists in the job; the restore's is not. An allowlist that names only registries denies the one fetch that has a hash to check. §15 D19.
...
1. `.spine/ci.sh` is byte-identical to §5.3 with the render tokens substituted, and no `@@` or `PIN_` token survives a render.
...
| R21 | "the allowlist lives in `.spine/ci.sh`, a floor path" (PB §7.1). | `ci.sh` **declares** `SPINE_ALLOWED_HOSTS` and configures the clients that honour a proxy; the isolation boundary **enforces** — and **v1 ships no enforcer**, because RF §7.1's M1 creates no network namespace (§5.6, RF §14 OPEN-9). SwiftPM is named as having no environment knob, its mirrors being frozen under `C-T2` instead; Gradle was named alongside it until 2026-08-27 and is gone with Kotlin, along with the two Maven/Gradle hosts the allowlist used to grant (§5.6). | A POSIX shell script cannot filter a socket, and a spec that implied otherwise would describe a control nobody implements. Stating the split is what makes the floor protection meaningful: the *declaration* is reviewed, the enforcement is the profile's, and PB §7.4 rule 5 already prices a weak profile. §15 D10. |
...
**D10 · OPEN · A POSIX shell script cannot enforce a network allowlist** (PB §7.1's untrusted row). *"network only to an allow-listed registry proxy during dependency restore … (the allowlist lives in `.spine/ci.sh`, a floor path)."* `ci.sh` can declare hosts and set client environment variables; the filtering is the container's, the proxy's or the firewall's. As written the sentence describes a control nobody implements. Recommended: say that `ci.sh` declares and the boundary enforces — which keeps the floor protection meaningful and matches how PB §7.4 rule 5 already prices a weak profile.

**D11 · OPEN · The registry allowlist excludes the one fetch that has a hash to check** (PB §7.1's untrusted row against PB §7.4 rule 2). Rule 2 has `.spine/ci.sh` install and hash-verify the collector; §7.1 grants the untrusted stage network "only to an allow-listed registry proxy during dependency restore, then none". The distribution host is not a registry proxy and the install is not a dependency restore, so a literal allowlist blocks the installer. Recommended: name the distribution root as a second, earlier grant, distinguished by the fact that its content is authenticated by a digest read from trunk.
...
- **The release's build, signing and publication process.** This document fixes the *layout and bytes* an installer must find (§5.5) and nothing about how they get there.
- **Which host serves the distribution root.** §18 OPEN-1.
...
- **Caching of dependencies.** The trusted job restores none (PB §7.4 rule 3). The untrusted job may, and a poisoned cache is inside the same residual as a lying runner: it can affect `out` values and nothing else the trusted stage reads.
...
**OPEN-1 · The distribution root.** §5.5 fixes the layout `<base>/<dist_hash>/artifacts.txt` and the list's bytes; the host is the owner's, and `@@DIST_BASE@@` is a render token rather than an invented hostname. Whichever host is chosen, two properties are load-bearing and should be stated in the release process: the path is content-addressed by the list's own SHA-256, and every artifact the list names is served from the same directory.
...
**OPEN-7 · Which action versions the release pins.** §3.3's three `PIN_` tokens. The versions themselves are a release decision; the *requirement* — full 40-hex commit ids, never tags — is not, and is normative here. **Recommendation:** pin, and re-pin only in a release, so that a repository's action set is as auditable as its spine binary.
