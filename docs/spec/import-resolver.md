...
8. **Every adapter names its terminal session-end event**, because `result-file.md` §7.3's `complete` is defined in terms of one: `pytest`'s is the `pytest_sessionfinish` hook, `dart-test`'s is the `done` event, and `swift-test`'s is the final `Test Suite 'All tests' <verb> at …` line. **`vitest`'s is not yet named** and is owed by §11.3 before that adapter ships; until it is, `complete` has no definition for a TypeScript suite and no `vitest` adapter is conformant.

   `pytest_sessionfinish` was chosen by measurement rather than from the documentation: verified against pytest 9.1.1 under CPython 3.14.5, it runs after the last item and **does not run when the process group is killed**, which is exactly the discrimination §7.3 asks for — a run that finished from one that died part-way. An adapter that inferred the terminal event from the stream simply ending could not tell them apart, because a killed process looks the same.
