...
- **Anchor:** an import site begins at a `word` token `import` or `from` that is the first token of a logical line, the first token after a `;`, or **the first token after a `:` that opens a compound statement's suite on the same logical line**. Both are keywords, so no further disambiguation is needed. The third clause is what reaches `try: import oracle` — §3.7 names that construct by name and requires that *"every branch of every conditional construct contributes its import sites"*, and an anchor that stopped at the first two left a one-line hiding place in the freeze closure, which is exactly the failure that cost Kotlin its place in v1.
...
2. The YAML is read as the **declarative subset**: block mappings, block sequences and plain or single/double-quoted scalars only. Anchors (`&`), aliases (`*`), merge keys (`<<`), tags (`!`), multi-document streams (`---` more than once) and flow mappings that nest more than one level → unclassifiable, reason `pubspec-not-declarative`.
   ...
   **A quoted scalar carries YAML's own escapes, and they are read.** Inside a single-quoted scalar, `''` is one literal `'`; inside a double-quoted scalar, `\"` is one literal `"` and `\\` one literal backslash. They are part of the scalar forms the subset already admits and are not an excluded construct. Refusing them was measured against the consequence: a refused pubspec is `pubspec-not-declarative`, which is `lang-unclassifiable`, and under §3.8's language level **every Dart file in the repository then contributes no edges** — so one apostrophe in a `description:` would block every Dart landing in that repository until a human noticed and rewrote the line. That is the blast radius of the `#`-inside-quotes defect this document already records, reached by a different route.
...
6. **`fn` is a prefix of `id`**, checked per record by `result-file.md` §4.4. Every v1 adapter satisfies it: `pytest` by construction, and `vitest`, `dart-test` and `swift-test` because `fn == id`.
7. **The six obligations of `result-file.md` §6.3 are discharged here for all four runners**, and nowhere else. Obligation 6 — a `B` outcome per collected id — is the newest and the only one with a per-landing price; §11.1 fixes what each adapter runs to satisfy it and what that costs. There is no `docs/spec/runner-adapters.md` and none is owed.
8. **Every adapter names its terminal session-end event**, because `result-file.md` §7.3's `complete` is defined in terms of one: `pytest`'s is the `pytest_sessionfinish` hook, `vitest`'s is the reporter's `onTestRunEnd`, `dart-test`'s is the `done` event, and `swift-test`'s is the final `Test Suite 'All tests' <verb> at …` line.

   **Both of the first two were chosen by measurement rather than from documentation**, and the measurement is the same in each case: the event must fire when the run finishes and **not** fire when the process group is killed, which is exactly the discrimination §7.3 asks for — a run that finished from one that died part-way. An adapter that inferred the terminal event from the stream simply ending could not tell them apart, because a killed process looks the same.

   `pytest_sessionfinish`, verified against pytest 9.1.1 under CPython 3.14.5: it runs after the last item and does not run under `SIGKILL`.

   `onTestRunEnd`, verified against vitest 4.1.11 under Node 26: a completed run ends `… onTestModuleEnd, onTaskUpdate, onTestRunEnd`, and the same suite killed mid-run ends `… onTestCaseReady, onTaskUpdate` with **no `onTestRunEnd` at all**. It also fires on a **red** suite and on a suite whose module fails to import, which is required rather than incidental — §7.3 says "The runner's *exit code* is never the discriminator — a red suite exits non-zero on every runner that ships, so an exit-code test would make `complete` unreachable for exactly the runs G1 exists to judge", and both of those runs exit non-zero.

   **Reporter API version.** `onTestRunEnd` is vitest's reporter interface, and a `vitest` reporter is a **class** rather than a plain object — an object export is rejected at startup with `TypeError: (intermediate value) is not a constructor`. The adapter ships its reporter in the pinned release for the same reason the pytest transport ships there (`result-file.md` §6.6): it must not come from the candidate's tree.
