...
**A target's source files.** If `sources:` is given, exactly the entries under the source directory (a file entry, or every `.swift` file recursively under a directory entry). Otherwise every `.swift` file recursively under the source directory. Then remove every path under any `exclude:` entry. A path belonging to two targets → unclassifiable, reason `overlapping-targets`.
...
Language level: `manifest-not-literal`, `duplicate-target-name`, `xcode-project-unsupported`, `target-dir-missing`, `overlapping-targets`, `no-package-manifest` (a Swift file exists and no `Package.swift` does), `rc-changed-on-branch`.
...
| **Swift** | **yes, within SwiftPM** | a literal `Package.swift`; no Xcode project | **Mixed Swift/Objective-C targets.** A target containing `.m`/`.h` files, or a bridging header, can hold an oracle the resolver cannot see: Objective-C is not one of the four languages, `lang` returns `none`, and no edge exists. The restriction is not as narrow as it looks — `swift test` requires SwiftPM, so a repository spine can gate is a SwiftPM repository by construction, and an Xcode-only project simply has no supported runner. |

Two things follow that the owner asked to hear plainly.

**Swift does meet the bar.** The configuration it needs is the configuration its runner needs. What it does not close is Objective-C interop, which is nameable, narrow, and could be closed later by adding an ObjC lexer — the resolution rules are the same (module-granular, `#import "…"` and a bridging header) and nothing in this document's structure would change. §18, OPEN-2 asks whether to detect and refuse it in the meantime.

**Kotlin does not appear in this table, and version 1's row for it is Appendix A.** Its restriction was the conventional Gradle subset, which fails *loudly* — `layout-overridden` is one lexical test. Its residual was mixed Kotlin/Java modules, which fail *silently*: a `.java` file in a Kotlin source set is `lang: none`, contributes no edges, is the target of none, and Kotlin's same-package visibility means a test reaches a Java helper with no import line at all. A silent hole in the freeze closure is the failure mode PB §4.3 exists to close, and mixed Kotlin/Java modules are ordinary rather than exotic. Version 1 recommended detect-and-refuse; the owner dropped the language instead (2026-08-26), which closes the hole rather than reporting it. Appendix A keeps the whole analysis so that a release adding Kotlin starts from it.
...
**Why a second invocation and not one.** `swift test list` prints specifiers and nothing else — no verb, no outcome, no `Expected failure` line — so it cannot supply `out`. And the datum the carve-out needs is `xfail`, which for this adapter comes from an `Expected failure in <case identity>:` line that `XCTExpectFailure` prints from **inside the test body** (below). There is no listing, no build product and no static read of the tree from which it can be predicted. Running the suite is the only answer XCTest offers.
...
4. **The `B` outcome is the adapter's own mapping, applied to `B`.** Obligation 6 of `result-file.md` §6.3 is discharged by running the adapter's *`T` invocation* against the checkout of `B` and passing its terminal reports through the **same** outcome mapping the adapter uses on `T` (obligation 4). No adapter defines a second mapping, a `B`-only value or a `B`-only refusal, and none may: `result-file.md` §8.5 clause 2 compares `b.out` against the single literal `xfail`, so a `B` mapping that differed from the `T` mapping would make the carve-out mean one thing on trunk and another on the branch. The one value the `T` side has no use for is `absent`, which is not produced by a mapping at all — it is what an id that the `B` outcome run reported no terminal outcome for gets (`result-file.md` §4.4), and it is written by the collector rather than by the adapter.
5. **`xfail` is producible by two of the four adapters, and only those two.** `pytest` produces it from the expected-failure marker (§11.2) and `swift-test` from an `Expected failure in` line (§11.5); `vitest` and `dart-test` have no expected-failure value in their mappings at all (§11.3, §11.4). So `result-file.md` §8.5 clause 2's carve-out is reachable only in a Python or a Swift suite — which is exactly the pair of adapters whose `B` enumeration reports no outcome, and is the whole reason obligation 6 costs a second invocation rather than nothing. An implementer who reads §11.1's cost table and wonders whether the cheap half could be skipped for the expensive runners has it backwards.
...
| S12 | A target containing a `.m` file | the `.m` file is `lang: none` and contributes nothing — the documented hole (§10, §18 OPEN-2) |
...
| R17a | Any `vitest` or `dart-test` `base` record | an `out` from that adapter's own mapping, never `xfail` — neither runner has an expected-failure value (§11.3, §11.4, §11.6 rule 5) |
...
**OPEN-2 · Swift's Objective-C hole.** A target containing `.m`/`.h` files or a bridging header can hold an oracle the resolver cannot see: Objective-C is `lang: none`, so no edge exists (§10). It is narrower than the Kotlin/Java hole that cost Kotlin its place — a Swift package that mixes Objective-C is less common than a Gradle module that mixes Java, and SwiftPM's own convention keeps ObjC in its own target — but it is the same *shape*, and the same three exits apply: detect and refuse, add the language, or drop Swift. **Recommendation:** detect and refuse — an `RC(swift, ·)` refusal, reason `mixed-objc-target`, raised when a target's source directory contains a `.m`, `.mm` or `.h` entry — which is one lexical test and turns a silent hole into a loud one. Owner-level because it makes a currently-gatable repository shape ungatable.
...
**OPEN-13 · Whether an id trunk itself skips belongs in the `B` floor.** §11.1's shared rule makes the floor the ids the runner collected and selected, irrespective of outcome, and every adapter here honours it: `dart-test` emits a `base` record for a skipped test (§11.4, row 5), `swift test list` lists a method that will throw `XCTSkip`, `pytest --collect-only` collects a `@pytest.mark.skip` item, and `vitest run` reports an `it.skip` case. `result-file.md` §8.5 clause 2 then requires a `passed` `result` record for every `base` record, so **a test skipped on trunk blocks every subsequent landing** until a `class=protected` `G8:<path>` review names its path — and it does so again on the next landing, because trunk has not changed. Skipped tests on trunk are ordinary. The alternative is to define the floor as *collected, selected and not skipped*, which loses nothing in strength — a test skipped on trunk is asserting nothing on trunk — and costs one more signal per adapter, which `result-file.md` §6.6's transport already preserves. **Recommendation:** exclude skipped ids from the floor, as deselected ones already are, for the same reason. Owner-level because it edits PB §4.3's *"every test id trunk collected on `B`"* and `result-file.md` §8.5 clause 2, not merely this section, and because it is the one place where a stricter floor is strictly worse.

**What version 4 changes about this, and what it deliberately does not.** The input OPEN-13 lacked now exists: every `base` record carries its `B` outcome (§11.1, `result-file.md` §4.4), so `skipped` on `B` is one literal comparison away from being actionable and costs no further invocation in any adapter. **The question is still open and this document has not answered it.** The `xfail` carve-out of `result-file.md` §8.5 clause 2 was written because PB §6.3 writes it in both the G1 and the G8 row; `skipped` appears in neither, and helping myself to it because the datum arrived would widen the floor's meaning under cover of a repair. A reader who sees `out` on a `base` record must not infer that a `skipped` id is exempt: as the corpus stands, an id trunk reports `skipped` on `B` is in the floor and its non-passing `result` record is a G8 and a G1 finding, exactly as before. `result-file.md` §14 OPEN-8 records the same boundary from the other side.
...
3. Resolution configuration is extracted from `B`; `RC(lang, A)` is computed only to compare (§3.3).
...
31b. `xfail` is producible on `B` by `pytest` and `swift-test` alone, so `result-file.md` §8.5 clause 2's carve-out is reachable in a Python or a Swift suite and nowhere else (§11.6 rule 5).
